...
public class AndroidTickTest {
    ...
    // 测试零延迟情况：应抛出 IllegalArgumentException
    @Test
    public void testZeroDelay_shouldThrowIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> new TestAndroidTick(0L));
    }

    // 测试负数延迟情况：应抛出 IllegalArgumentException
    @Test
    public void testNegativeDelay_shouldThrowIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> new TestAndroidTick(-100L));
    }
    ...
    // 新增测试：测试cancel后再次tick（计数继续递增，不重置）
    @Test
    ...
    // 带状态检查的测试用子类（直接复用基类公开的 isRunning()）
